    ...
    <section className="py-20 sm:py-28">
      <div className="mx-auto max-w-7xl px-5 sm:px-8">
        ...
        <div className="mx-auto mt-14 max-w-4xl overflow-hidden rounded-2xl border border-white/[0.08]">
          {/* Column headers — shown as a table header from sm up. On mobile
              each row becomes a self-labeled card instead, so the columns are
              never squeezed side-by-side on a narrow screen. */}
          <div className="hidden grid-cols-2 border-b border-white/[0.08] sm:grid">
          ...
          {/* Rows: stacked, self-labeled cards on mobile; two columns from sm up. */}
          {t.comparison.rows.map((row, i) => (
            <div
              key={row.safunded}
              className={`flex flex-col sm:grid sm:grid-cols-2 ${i % 2 === 0 ? "bg-white/[0.02]" : "bg-transparent"}`}
            >
              <div className="flex items-start gap-3 border-b border-white/[0.06] px-5 py-4 sm:border-b-0 sm:px-6 sm:py-5">
